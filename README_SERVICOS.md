...
- **OficialTelnetServicePySocket.mq5** (**PyInService / pyin**) – **escuta TCP em 9091**. Suporta `PY_CALL` / `PY_ARRAY_CALL` e frames binários `SEND_ARRAY`/`GET_ARRAY` (PyOutService em 9100).
...
## Python
- **python/python_bridge_server.py** (**PyOutService / pyout**) – **server** em 0.0.0.0:9100 (default).
...
- “PyOutService / pyout” → python/python_bridge_server.py (server default)
- “PyBridge SDK” → mt5/Services/OficialTelnetServicePySocket/PyBridge.mqh + python/mt5_bridge.py

## Mini‑SDK (facilitar MQL↔Python)
- **MQL**: `mt5/Services/OficialTelnetServicePySocket/PyBridge.mqh` (PyInService / pyin)
...
- Bootstrap: `integrations/python-mql/notes/MT5_BOOTSTRAP_SERVICE.md`
