...
int BuildParamsLocal(string &pstr, MqlParam &outParams[]);
...
double ToDoubleSafe(const string v) { return StringToDouble(v); }
int ToIntSafe(const string v) { return (int)StringToInteger(v); }
...
bool H_OpenChart(string &p[], string &m, string &d[])
{
  ...
  if(cid==0){ m=StringFormat("ChartOpen fail err=%d", GetLastError()); return false; }

  // opcional: traz o chart pro topo (assíncrono)
...
bool H_AttachInd(string &p[], string &m, string &d[])
{
  ...
  g_lastIndName=name; g_lastIndParams=""; g_lastIndSymbol=sym; g_lastIndTf=tfstr; g_lastIndSub=sub;
...
bool H_DetachInd(string &p[], string &m, string &d[])
{
  ...
  for(int i=total-1;i>=0;i--)
  {
    ...
    if(StringCompare(iname, name)==0)
      ChartIndicatorDelete(cid, sub-1, i);
  }
  m="indicator detached";
  return true;
}
...
{
  long cid=ChartID();
  int subs=(int)ChartGetInteger(cid, CHART_WINDOWS_TOTAL);
  for(int s=0;s<subs;s++)
  {
    int total=ChartIndicatorsTotal(cid, s);
    for(int i=total-1;i>=0;i--)
      ChartIndicatorDelete(cid, s, i);
  }
...
{
  long cid = (ArraySize(p)>0 && p[0]!="") ? (long)StringToInteger(p[0]) : ChartID();
  ChartRedraw(cid);
...
bool H_Screenshot(string &p[], string &m, string &d[])
{
  ...
  int width = (ArraySize(p)>3) ? (int)StringToInteger(p[3]) : 0;
  int height= (ArraySize(p)>4) ? (int)StringToInteger(p[4]) : 0;
  long cid=ChartOpen(sym, tf); if(cid==0){ m="ChartOpen"; return false; }
...
bool H_AttachEA(string &p[], string &m, string &d[])
{
  ...
  int handle=iCustom(sym, tf, "Experts\\"+expert);
...
bool H_DetachEA(string &p[], string &m, string &d[])
{
  ...
  for(int i=total-1;i>=0;i--)
  {
    ...
    {
      if(ChartIndicatorDelete(cid,0,i)) removed=true;
    }
  }
...
bool H_ObjMove(string &p[], string &m, string &d[])
{
  if(ArraySize(p)<3){ m="params"; return false; }
  long cid=ChartID();
  string name=p[0]; datetime t=StringToTime(p[1]); double price=StringToDouble(p[2]);
...
bool H_ObjCreate(string &p[], string &m, string &d[])
{
  ...
  double   p1=StringToDouble(PayloadGet(payload,"price"));
  datetime t2=StringToTime(PayloadGet(payload,"time2"));
  double   p2=StringToDouble(PayloadGet(payload,"price2"));
...
{
  if(ArraySize(p)<2){ m="params"; return false; }
  string sym=p[0]; double vol=StringToDouble(p[1]);
  if(!EnsureSymbol(sym)) { m="symbol"; return false; }
  if(vol<=0){ m="volume"; return false; }
  double sl = (ArraySize(p)>2 && p[2]!="") ? StringToDouble(p[2]) : 0;
  double tp = (ArraySize(p)>3 && p[3]!="") ? StringToDouble(p[3]) : 0;
...
bool H_TradeList(string &p[], string &m, string &d[])
{
  ...
  for(int i=0;i<total;i++)
  {
    // PositionGetTicket seleciona automaticamente a posição corrente para PositionGet*()
    ulong ticket=PositionGetTicket(i);
