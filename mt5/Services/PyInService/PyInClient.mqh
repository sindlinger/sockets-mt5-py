// PyInClient.mqh - mini SDK MQL para serviço PyIn (9091)
#ifndef __PYIN_CLIENT_MQH__
#define __PYIN_CLIENT_MQH__

#include "PyInSockServer.mqh"
...
// Async: envia array e recebe job_id
bool PyBridgeSubmitF64(const double &arr[], int count, const string func,
                       string &job_id,
                       const string host=PYBR_DEFAULT_HOST, const int port=PYBR_DEFAULT_PORT,
                       string &err="")
{
  ...
  if(!PyBridgeDoublesToBytes(arr, count, raw)) { err="pack"; PyBridgeClose(sock); return false; }
  string id = IntegerToString(GetTickCount());
  string header = id+"|PY_ARRAY_SUBMIT|"+func+"|f64|"+IntegerToString(count)+"|"+IntegerToString(ArraySize(raw));
  if(!PyBridgeSendFrame(sock, header, raw)) { err="send"; PyBridgeClose(sock); return false; }
  string rh=""; uchar payload[];
  if(!PyBridgeRecvFrame(sock, rh, payload)) { err="resp"; PyBridgeClose(sock); return false; }
  PyBridgeClose(sock);
  string hp[]; int hn=StringSplit(rh,'|',hp);
  if(hn<6) { err="bad_header"; return false; }
  if(hp[1]=="PY_ARRAY_ACK") { job_id=hp[2]; return true; }
  if(hp[1]=="PY_ARRAY_ERROR")
  {
    err = CharArrayToString(payload,0,ArraySize(payload),CP_UTF8);
    if(err=="") err="py_error";
    return false;
  }
  err="bad_resp";
...
// Async: consulta job_id; retorno: 1=done, 0=pending, -1=error
int PyBridgePollF64(const string job_id, double &out[],
                    const string host=PYBR_DEFAULT_HOST, const int port=PYBR_DEFAULT_PORT,
                    string &err="")
{
  err="";
  uint sock=0;
  if(!PyBridgeConnect(host, port, sock)) { err="conn"; return -1; }
  uchar empty[]; ArrayResize(empty,0);
  string id = IntegerToString(GetTickCount());
  string header = id+"|PY_ARRAY_POLL|"+job_id+"|f64|0|0";
  if(!PyBridgeSendFrame(sock, header, empty)) { err="send"; PyBridgeClose(sock); return -1; }
  string rh=""; uchar payload[];
  if(!PyBridgeRecvFrame(sock, rh, payload)) { err="resp"; PyBridgeClose(sock); return -1; }
  PyBridgeClose(sock);
  string hp[]; int hn=StringSplit(rh,'|',hp);
  if(hn<6) { err="bad_header"; return -1; }
  if(hp[1]=="PY_ARRAY_PENDING") return 0;
  if(hp[1]=="PY_ARRAY_RESP")
  {
    int out_count=(int)StringToInteger(hp[4]);
    if(out_count<=0) { err="count"; return -1; }
    if(!PyBridgeBytesToDoubles(payload, out_count, out)) { err="unpack"; return -1; }
    return 1;
  }
  if(hp[1]=="PY_ARRAY_ERROR")
  {
